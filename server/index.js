...
app.post('/api/maintenance/config', async (req, res) => {
    const { name, limit_km, last_service_km } = req.body;
    const secretKey = req.headers['x-secret-key'];
    if (secretKey !== process.env.SECRET_KEY) return res.status(401).json({ error: 'Unauthorized' });
    const { data, error } = await supabase
        .from('maintenance_configs')
        .insert([{ name, threshold_km: parseInt(limit_km), last_service_km: parseInt(last_service_km) }]);
...
app.delete('/api/maintenance/config/:id', async (req, res) => {
    ...
    const secretKey = req.headers['x-secret-key'];
    if (secretKey !== process.env.SECRET_KEY) return res.status(401).json({ error: 'Unauthorized' });
    const { error } = await supabase.from('maintenance_configs').delete().eq('id', id);
    if (error) { console.error('Помилка видалення:', error); return res.status(500).json(error); }
    res.json({ success: true });
});
...
// ══════════════════════════════════════════════════════════════
// КОНСТАНТИ КАТЕГОРІЙ
// ЗМІНА 1: Додано 'Догляд' та 'Подарунки' перед 'Інше' у витратах.
// Ці категорії автоматично з'являться у боті та в savingsExpenseScene.
// ══════════════════════════════════════════════════════════════
const CATEGORIES = {
    EXPENSES: ['Продукти', 'Медицина', 'Розваги', 'Одяг', 'Машина', 'Квартира', 'Податки', 'Кредити', 'Догляд', 'Подарунки', 'Інше'],
    INCOME: ['Школа', 'Хелен Дорон', 'Лангейт', 'Інші'],
...
async function getCarData() {
    ...
    if (recentFuel.length >= 2) {
        const totalLiters = recentFuel.slice(1).reduce((sum, f) => sum + f.liters, 0);
...
async function saveCarData(entry) {
    ...
    const { error } = await supabase.from('car_stats').insert([dbEntry]);
...
async function saveMaintenanceData(entry) {
    ...
    const { error } = await supabase.from('car_maintenance').insert([dbEntry]);
    if (error) console.error('Supabase maintenance insert error:', error);
}

// ── Логіка Telegram Бота (авто) ──
if (TELEGRAM_TOKEN) {
    const mainKeyboard = {
        reply_markup: {
            keyboard: [
    ...
    bot.hears('🗑 Скасувати останній запис', async (ctx) => {
        const { data: fuelData } = await supabase
            .from('car_stats').select('*').order('id', { ascending: false }).limit(1);
        const { data: maintData } = await supabase
            .from('car_maintenance').select('*').order('id', { ascending: false }).limit(1);
        ...
        if (f && m) {
            ...
            if (fTime > mTime) { last = f; type = 'fuel'; }
            else if (mTime > fTime) { last = m; type = 'service'; }
            else { last = m; type = 'service'; }
        } else {
            last = f || m;
            type = f ? 'fuel' : 'service';
        }
    ...
    bot.command('service', async (ctx) => {
        ...
        if (numbers && numbers.length >= 2) {
            ...
            let description = messageText
                .replace(numbers[0], '').replace(numbers[1], '')
                .trim().replace(/\s+/g, ' ');
            if (!description) description = "Ремонт/Обслуговування";
            const entry = { date: new Date().toISOString().split('T')[0], odo, description, cost, type: 'Ремонт' };
            try {
    ...
    bot.on('text', async (ctx) => {
        ...
        if (parts.length >= 2 && !isNaN(parts[0]) && !isNaN(parts[1])) {
            ...
            try {
                ...
                const entry = { date: new Date().toISOString(), amount, odo, liters, priceAtTime: price };
                if (!isTest) await saveCarData(entry);
...
if (FINANCE_TOKEN) {
    ...
    // ══════════════════════════════════════════════════════════════
    // ЗМІНА 2 (ВИПРАВЛЕННЯ БАГА): Визначення типу транзакції.
    //
    // ПРОБЛЕМА: Категорія 'Інше' є і в EXPENSES, і в INCOME.
    // Стара логіка в transactionScene перевіряла INCOME першою:
    //   if (CATEGORIES.INCOME.includes(cat)) type = 'Доходи';
    // Тому вибір 'Інше' у витратах завжди давав тип 'Доходи'.
    //
    // ВИПРАВЛЕННЯ: Функція getTransactionType перевіряє EXPENSES
    // першими. Якщо категорія є у витратах — це витрата, крапка.
    // ══════════════════════════════════════════════════════════════
    const getTransactionType = (category) => {
        if (CATEGORIES.EXPENSES.includes(category)) return 'Витрати';
        if (CATEGORIES.SAVINGS.includes(category)) return 'Заощадження';
        if (CATEGORIES.INCOME.includes(category)) return 'Доходи';
        return 'Витрати';
    };
    ...
    const transactionScene = new Scenes.WizardScene(
        ...
        async (ctx) => {
            ...
            ctx.scene.session.state.category = cat;
            // ВИПРАВЛЕНО: використовуємо getTransactionType
            ctx.scene.session.state.type = getTransactionType(cat);
    ...
    // Сцена для витрат ІЗ ЗАОЩАДЖЕНЬ
    const savingsExpenseScene = new Scenes.WizardScene(
        'SAVINGS_EXPENSE_SCENE',
        async (ctx) => {
            ctx.scene.session.state.currency = ctx.message.text.split(' ')[0];
            // Кнопки автоматично включають нові категорії Догляд і Подарунки
    ...
    if (finBot && GROUP_CHAT_ID) {
        ...
        finBot.action(/^delete_(.+)$/, async (ctx) => {
            ...
            const { data: record } = await supabase
                .from('family_finances').select('*').eq('id', recordId).single();
            if (record) {
                const { error: deleteError } = await supabase
                    .from('family_finances').delete().eq('id', recordId);
                if (!deleteError) {
                    await ctx.answerCbQuery('Видалено!');
        ...
        finBot.on('text', async (ctx, next) => {
            ...
            const [_, catInput, amountStr] = match;
            ...
            try {
                console.log(`[finBot] Записую: ${category} -> ${amount} ${currency} (${type})`);
                await supabase.from('family_finances').insert([{
                    user_id: ctx.from.id, type, category, amount, currency
                }]);
...
app.post('/api/maintenance/config', async (req, res) => {
    ...
    try {
        if (id) {
            await supabase.from('maintenance_configs')
                .update({ name, threshold_km, last_service_km }).eq('id', id);
        } else {
            await supabase.from('maintenance_configs')
...
app.post('/api/maintenance/done', async (req, res) => {
    ...
    try {
        const { error } = await supabase.from('maintenance_configs')
            .update({ last_service_km: currentOdo }).eq('id', configId);
        if (error) throw error;
        res.sendStatus(200);
    } catch (e) {
        res.status(500).send(e.message);
...
app.get('/api/dashboard', async (req, res) => {
    ...
    try {
        ...
        history.forEach(t => {
            if (t.type === 'Заощадження') safe[t.currency] += t.amount;
            if (t.is_from_savings) safe[t.currency] -= t.amount;
        });
